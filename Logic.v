...
Proof.
  ...
  split.
  intro P1. apply HQR;apply HPQ;apply P1.
  intro R1. apply HQP;apply HRQ;apply R1.
Qed.
...
let H0 :=
  match H with
  | or_introl HP => let Case := "left" in or_intror Q P HP
  | or_intror HQ => let Case := "right" in or_introl Q P HQ
  end in
H0
     : forall P Q : Prop, P \/ Q -> Q \/ P
*)
...
Proof. 
  intros b c H.
  destruct b.
Case "b=true". destruct c. inversion H. inversion H.
Case "b=false". destruct c. inversion H. split. reflexivity. reflexivity.
Qed.
...
Proof.
  intros P H.
  inversion H as [H1| H2].
  unfold not. intros H'. apply H1.
  unfold not. intros H'.  apply H' in H2. inversion H2.
Qed.
...
Proof.
  induction n.
Case "n=0". destruct m.
  SCase "m=0". intros H. unfold not. inversion H.
  SCase "m=S m". intros H. unfold not. intros H1. inversion H1.
Case "n=S n". destruct m.
  SCase "m=0". intros H. unfold not. intros H1. inversion H1.
  SCase "m=Sm". simpl. intros H. apply IHn in H. unfold not in H.
             unfold not. intro H0. apply H. inversion H0. reflexivity.
Qed.
...
Proof.
   ...
   unfold iff.
   split.
Case "(exists x : X, P x \/ Q x) -> (exists x : X, P x) \/ (exists x : X, Q x)".
   intros H. inversion H. 
   inversion H0.
   left. exists witness. apply H1.
   right. exists witness. apply H1.
Case "(exists x : X, P x) \/ (exists x : X, Q x) -> exists x : X, P x \/ Q x".
   intros H. inversion H. 
   inversion H0. 
   exists witness.
   left. apply H1. 
...
| total : forall m : nat, total_relation n m.
...
Proof.
  ...
  apply all_nil.
  apply all_cons.
  apply (andb_true_elim1 (test x) (forallb test l)).
  apply H.
  apply IHl.
  apply (andb_true_elim2 (test x) (forallb test l)).
  apply H.
Qed.
...
|merge0:forall l1:list nat,list_merge l1 nil l1
|merge1:forall l2:list nat,list_merge nil l2 l2
|merge2:forall (l1 l2 l3:list nat)(a b :nat),
        list_merge l1 l2 l3->list_merge (a::l1) (b::l2) (a::b::l3).

Theorem merge_t1:list_merge [1,6,2] [4,3] [1,4,6,3,2].
Proof.
 apply merge2. 
 apply merge2.
 apply merge0.
...
simpl.
rewrite H3.
assert (filter  f l3 = l1 -> a :: filter  f l3 = a :: l1).
intros HX.
rewrite HX.
reflexivity.
...
(forall a, appears_in a l1 ->
 ~(appears_in a l2)) /\ (forall a, appears_in a l2 -> ~(appears_in a l1)).
...
| repeats_in : forall a l, no_repeats l -> 
             ~(appears_in a l) -> no_repeats (a :: l).
...
| nost_nil : nostutter []
| nost_single : forall (n : nat), nostutter [n]
| nost_diff : forall (n m : nat) (l : list nat), n <> m ->
     nostutter (m :: l) -> nostutter (n :: m :: l).
...
| rep_appears : forall (x : X) (l : list X), appears_in x l -> repeats (x :: l)
| rep_skip : forall (x : X) (l : list X), repeats l -> repeats (x :: l).
...
Lemma diff_rem : forall (X : Type) (x x0 : X) (l l' : list X), excluded_middle -> x <> x0 -> appears_in x0 (l ++ x :: l') -> appears_in x0 (l ++ l').
Proof.
  induction l.
  intros.
  simpl.
...
Proof.  intros X l1. induction l1.
  ...
  apply H3.
  apply rep_skip.
  assert (exists l, exists l', l2 = l ++ (x::l')).
  ...
  apply H7.
  subst.
  apply (diff_rem X x x0 witness witness0).
  unfold excluded_middle.
  apply H.
  apply H9.
  apply H10.

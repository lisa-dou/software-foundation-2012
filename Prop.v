...
Proof.
 assert(3+3=6). auto.
 rewrite <-H. apply b_sum; constructor.
Qed.
...
Theorem nine_is_beautiful :
  beautiful 9.
Proof.
  assert (3+6=9) by auto.
  rewrite <- H . apply b_sum. constructor. apply six_is_beautiful.
Qed.
...
let H0 := f_equal (plus n) (plus_n_O n) in
eq_ind (n + n) (fun n0 : nat => beautiful n0) (b_sum n n H H) (2 * n) H0.
...
Proof.
   intros n H.
   replace (13+n) with (5+5+3+n).
   apply g_plus5 with (n:=5+3+n).
   apply g_plus5 with (n:=3+n). apply g_plus3. assumption.
   auto.
Qed.
...
let H0 := eq_refl (13 + n) in
eq_ind (5 + 5 + 3 + n) (fun n0 : nat => gorgeous n0)
  (g_plus5 (5 + 3 + n) (g_plus5 (3 + n) (g_plus3 n H))) (13 + n) H0.
...
Proof.
  ...
  apply g_plus3 with (n:=0);constructor.
  apply g_plus5 with (n:=0);constructor.
...
Proof.
   intros x y z .
   rewrite plus_assoc'.
   assert (x+z=z+x). apply plus_comm.
   rewrite H. auto.
Qed.
...
*)
 Inductive pal : forall (X : Type) (l : list X), Prop :=
  | pal_empty : forall X, pal X []
  | pal_single : forall X (x : X), pal X [x]
  | pal_append : forall X (x : X) (l : list X), pal X l -> pal X (x :: (snoc l x)).

Theorem pal_1 : forall (X : Type) (l : list X), pal X (l ++ rev l).
  Proof.
    intros.
    induction l.
    apply pal_empty.
...
Theorem pal_2 : forall (X : Type) (l : list X), pal X l -> l = rev l.
  Proof.
    intros.
    induction H.
    reflexivity.
...
Inductive subseq : forall (a b : list nat), Prop :=
  | sub_empty : forall (b :list nat), subseq [] b
  | sub_equal : forall (a b : list nat), subseq a b -> forall (x : nat), subseq (x :: a) (x :: b)
  | sub_skip : forall (a b : list nat), subseq a b -> forall (x : nat), subseq a (x :: b).

Theorem subseq_refl : forall (X : Type) (a : list nat), subseq a a.
Proof.
    intros.
    induction a.
    apply sub_empty.
...
Theorem subseq_left : forall (l1 l2 l3 : list nat), subseq l1 l2 -> subseq l1 (l2 ++ l3).
Proof.
    intros.
...
Theorem subseq_one : forall (x : nat) (l1 l2 : list nat), subseq (x :: l1) l2 -> subseq l1 l2.
Proof.
    intros.
...
Theorem subseq_transitive : forall (l1 l2 : list nat), subseq l1 l2 -> forall (l3 : list nat), subseq l2 l3 -> subseq l1 l3.
Proof.
    induction 1; simpl.
    intros.
    apply sub_empty.

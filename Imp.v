...
Fixpoint optimize_bexp_0plus (b:bexp) {struct b} : bexp :=
  match b with 
  | BTrue => BTrue
  | BFalse => BFalse
...
Fixpoint optimizer (e : aexp) : aexp :=
    match e with
      ...
      | AMult (ANum 1) e2 => optimizer e2
      | AMult e1 (ANum 1) => optimizer e1
...
Theorem optimizer_sound : forall e,
    aeval (optimizer e) = aeval e.
  Proof.
    aexp_cases (induction e) Case;
    try (simpl; rewrite IHe1; rewrite IHe2; reflexivity);
      try reflexivity.
    Case "APlus".
    aexp_cases (destruct e1) SCase;
    try (simpl; simpl in IHe1; rewrite IHe1; rewrite IHe2; auto).
    SCase "ANum".
    destruct n; destruct e2; simpl; auto.
    SCase "APlus".
    aexp_cases (destruct e2) SSCase;
    try (simpl; simpl in IHe2; rewrite IHe1; rewrite IHe2; auto).
    SSCase "ANum".
    ...
    inversion IHe1; subst; clear IHe1.
    rewrite H0.
    inversion IHe2; subst; clear IHe2.
    rewrite H1.
    omega.
    SSCase "AMinus".
    simpl.
    ...
    Case "AMinus".
    aexp_cases (destruct e2) SCase;
    try (simpl; simpl in IHe2; rewrite IHe2; rewrite IHe1; auto).
    SCase "ANum".
    destruct n; simpl; auto.
    destruct e1;
      try auto;
        try (rewrite IHe1; omega).
    destruct e1; simpl; auto.
    SCase "APlus".
    aexp_cases (destruct e1) SSCase;
    try (simpl; simpl in IHe2; rewrite IHe2; rewrite IHe2; auto).
    SSCase "ANum".
    destruct n; simpl; auto.
    inversion IHe2; subst; clear IHe2.
    rewrite H0.
...
Inductive bevalR : bexp -> bool -> Prop :=
  ...
  | E_BEq : forall (e1 e2 : aexp) (n1 n2 : nat), (e1 || n1) -> (e2 || n2) -> ((BEq e1 e2) ||| (beq_nat n1 n2))
  | E_BLe : forall (e1 e2 : aexp) (n1 n2 : nat), (e1 || n1) -> (e2 || n2) -> ((BLe e1 e2) ||| (ble_nat n1 n2))
  | E_BNot : forall (e1 : bexp) (b1 : bool), (e1 ||| b1) -> ((BNot e1) ||| (negb b1))
  | E_BAnd : forall (e1 e2 : bexp) (b1 b2 : bool), (e1 ||| b1) -> (e2 ||| b2) -> ((BAnd e1 e2) ||| (andb b1 b2))
...
Theorem beval_iff_bevalR : forall e b, (e ||| b) <-> beval e = b.
Proof.
    split.
    Case "->".
    intros H; induction H; subst; simpl; auto;
      replace (aeval e1) with n1; replace (aeval e2) with n2; auto;
        try (symmetry; apply aeval_iff_aevalR; apply H0);
          try (symmetry; apply aeval_iff_aevalR; apply H).
    Case "<-".
...
Proof.
  intros. 
  unfold not. intros.
  rewrite H0 in H. rewrite <-beq_id_refl in H. inversion H.
Qed.
...
Proof.
  ...
  auto.
  eapply E_Seq. eapply E_Ass. auto. eapply E_Ass. auto.
  apply E_WhileLoop with (update
     (update (update (update (update (update empty_state X 2) Y 0) Y 2) X 1)
        Y 3) X 0).
  auto. eapply E_Seq. eapply E_Ass. auto. eapply E_Ass. auto.
...
| NW_Ass : forall l a, no_whilesR ( l::=a )
| NW_Seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 ->no_whilesR (c1;c2)
| NW_IF  : forall b1 ct cf,no_whilesR ct -> no_whilesR cf ->
               no_whilesR (IFB b1 THEN ct ELSE cf FI).
...
induction c.
Case "left to right".
  SCase "SKIP". simpl. intros. apply NW_SKIP.
  SCase "Assignment". simpl. intros. apply NW_Ass.
  SCase "Sequence".  simpl.  intros. apply  NW_Seq.  apply andb_true in H;apply IHc1;elim H;auto. 
           apply andb_true in H; apply IHc2;elim H;auto.
  SCase "If ". simpl. intros. apply NW_IF. apply andb_true in H;apply IHc1;elim H;auto. 
           apply andb_true in H; apply IHc2;elim H;auto.
  SCase "While". intros. simpl in H. inversion H.
Case "right to left".
  induction c.
  SCase "skip". intros. simpl. reflexivity.
  SCase "i::=a". intros. simpl. reflexivity.
  SCase "seq". intros. simpl. inversion H. apply IHc1 in H2. apply IHc2 in H3. 
               rewrite H2. rewrite H3. reflexivity.
  SCase "if". intros. inversion H. simpl. apply IHc1 in H2. apply IHc2 in H4.
               rewrite H2. rewrite H4. reflexivity.
  SCase "while". intros. simpl. inversion H.
Qed.
...
Theorem no_whiles_terminating : forall c : com, no_whilesR c ->
  forall st : state, exists st', ceval c st st'.
Proof.
  induction c; intros.
  exists st; constructor; auto.
  exists (update st i (aeval st a)); constructor; auto.
  inversion H; subst.
  remember (IHc1 H2 st).
  inversion e; subst.
  remember (IHc2 H3 x).
  inversion e; subst.
  exists x0.
  apply (E_Seq c1 c2 st x x0); auto.
  inversion H; subst.
  remember (beval st b).
  destruct b0; subst.
  remember (IHc1 H2 st).
...
Theorem ss_body_preserves_invariant : forall st st' x z,
  ss_invariant x z st ->
  st X <> 0 ->
  subtract_slowly_body / st || st' ->
    ss_invariant x z st'.
Proof.
  unfold ss_invariant, subtract_slowly_body.
  intros.
  inversion H1; subst; clear H1.
...
Proof.
  ...
  ceval_cases (induction H0) Case; inversion Heqc; subst; clear Heqc; auto.
  apply IHceval2.
  apply (ss_body_preserves_invariant st st' x z).
  auto.
...
Theorem ss_com_correct : forall st st' x z,
  st X = x ->
  st Z = z ->
  subtract_slowly / st || st' ->
    st' Z = minus z x.
Proof.
  ...
  inversion H4; subst; clear H4.
  simpl in H8.
  remember (update (update st Z (st Z - 1)) X (update st Z (st Z - 1) X - 1)).  
  assert (ss_invariant (st X) (st Z) s).
  subst. unfold ss_invariant, update. simpl.
  destruct (st X).
  inversion H0.
  omega.
  assert (ss_invariant (st X) (st Z) st').
  apply ss_loop_preserves_invariant with s; auto.
  inversion H1; subst; clear H1.
  apply guard_false_after_loop in H8. simpl in H8.
  destruct (st' X).
  omega.
  inversion H8.
Qed.
...
Proof.
  ...
  destruct a; simpl; inversion H; subst; clear H1.
...
Proof.
  ...
  rewrite (s_execute_app st (s_compile e1) (s_compile e2 ++ [SPlus]) [] [aeval st e1] [aeval st e1 + aeval st e2]); auto.
  rewrite (s_execute_app st (s_compile e2) [SPlus] [aeval st e1] [aeval st e2, aeval st e1] [aeval st e1 + aeval st e2]); auto.
  ...
  rewrite (s_execute_app st (s_compile e1) (s_compile e2 ++ [SMinus]) [] [aeval st e1] [aeval st e1 - aeval st e2]); auto.
  rewrite (s_execute_app st (s_compile e2) [SMinus] [aeval st e1] [aeval st e2, aeval st e1] [aeval st e1 - aeval st e2]); auto.
  apply (s_execute_stack st (s_compile e2) [] [aeval st e1] [aeval st e2]).
  auto.
  intro; inversion H.
  intro; inversion H.

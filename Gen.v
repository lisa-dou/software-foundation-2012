...
Proof.
  ...
  inversion Eq.
  inversion Eq.
  rewrite <- (plus_n_Sm n n) in H0.
  rewrite <- (plus_n_Sm m m) in H0.
  inversion H0.
  rewrite -> (IHm n H1).
  reflexivity.
Qed.
...
Proof.
  intros X n l.
  generalize dependent n.
  induction l as [| x l']; intros n Eq; destruct n.
  reflexivity.
  inversion Eq.
  inversion Eq.
  inversion Eq.
  ...
  simpl.
  rewrite <- plus_n_Sm.
  rewrite -> (app_length_cons' X l' l' x (S (n + n))).
  reflexivity.
  rewrite -> (IHl' n H0).
  reflexivity.
Qed.

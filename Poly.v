...
Fixpoint split1 {X Y: Type} (s : list (X * Y)) : (list X)*(list Y) :=
match s with
	| nil => (nil, nil)
...
Proof.
  ...
  inversion Eq.
  simpl.
  assert (combine (fst (split l')) (snd (split l')) = l') as H.
  apply (IHl' (fst (split l')) (snd (split l'))).
  destruct (split l') as (a, b).
  reflexivity.
  rewrite -> H.
  reflexivity.
Qed.

Theorem split_combine : forall X Y (lx : list X) (ly : list Y),
  length lx = length ly -> split (combine lx ly) = (lx, ly).
Proof.
  intros X Y lx.
  ...
  inversion H0.
  intros ly Eq.
  destruct ly as [| y ly']; inversion Eq.
  simpl.
  rewrite -> (IHlx' ly' H0).
  reflexivity.
Qed.
...
Proof.
  ...
  unfold override.
  remember (beq_nat k1 k2) as e; destruct e.
  rewrite <- Eq.
  rewrite -> ((beq_nat_eq k1 k2) Heqe).
  reflexivity.
  reflexivity.
Qed.
...
Proof.
  ...
  inversion H.
  intros lf Eq.
  remember (test x0) as e; destruct e.
  inversion Eq.
  rewrite <- Heqe in H0.
  inversion H0.
  rewrite <- H1.
...
Proof.
  intros n m p Eq Eq'.
  rewrite -> ((beq_nat_eq n m) Eq).
  rewrite -> ((beq_nat_eq m p) Eq').
  apply beq_nat_refl.
Qed.
...
Proof.
  ...
  unfold override.
  remember (beq_nat k1 k3) as e1; destruct e1.
  remember (beq_nat k2 k3) as e2; destruct e2.
  rewrite <- ((beq_nat_eq k1 k3) Heqe1) in Heqe2.
  rewrite <- Heqe2 in Eq.
  inversion Eq.
  reflexivity.
  reflexivity.
...
Proof. reflexivity. Qed.
Example forallb_example2 : forallb negb [false,false] = true.
Proof. reflexivity. Qed.
Example forallb_example3 : forallb evenb [0,2,4,5] = false.
Proof. reflexivity. Qed.
Example forallb_example4 : forallb (beq_nat 5) [] = true.
Proof. reflexivity. Qed.
...
Theorem existsb_correct : forall X (test : X -> bool) (l : list X),
...
Proof.
  ...
  simpl.
  rewrite -> IHl.
  remember (test x) as e; destruct e.
  simpl.
  unfold existsb'.
  simpl.
  rewrite <- Heqe.

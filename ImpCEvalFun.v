...
induction i1 as [|i1']; intros i2 st st' c Hle Hceval.
  ...
  Case "i1 = S i1'".
    ...
    SCase "SKIP".
      ...
      reflexivity.
    SCase "::=".
      simpl in Hceval. inversion Hceval. simpl.
      reflexivity.
    SCase ";".
      ...
      SSCase "st1'o = Some".
        ...
        apply (IHi1' i2') in Hceval. assumption. assumption.
      SSCase "st1'o = None".
        inversion Hceval.
...
(*1*)
  exists 1. simpl. auto.
(*2*)
  exists (S n). simpl. rewrite H. auto.
(*3*)
  destruct IHHce1; destruct IHHce2.
  exists (x + x0). 
    destruct x. inversion H.
    simpl. destruct x0. inversion H0.
    assert (S x <= x + S x0).
    rewrite <- plus_Snm_nSm. apply le_plus_l.
    assert (ceval_step st c1 (x + S x0) = Some st') as Ha2.
        apply ceval_step_more with (S x).
        assumption. assumption.        
      rewrite -> Ha2. 
      assert (S x0 <= x + S x0). 
        apply le_plus_r. 
      apply ceval_step_more with (S x0).
      assumption. assumption.
(*4*)
  destruct IHHce. 
   exists (S x). simpl. rewrite H. assumption.
(*5*)
  destruct IHHce.
   exists (S x). simpl. rewrite H. assumption.
(*6*)
  exists (S 0). simpl. rewrite H. auto.
(*7*)
  destruct IHHce1.   destruct IHHce2.
  ...
  omega. assumption.
  rewrite H2. apply ceval_step_more with x0.
  omega. assumption.
Qed. 
